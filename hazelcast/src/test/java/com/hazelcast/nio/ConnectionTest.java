...
public class ConnectionTest extends HazelcastTestSupport {
    ...
    public void testBlockedClientSockets() throws Exception {
        ...
        final Thread st = Thread.ofVirtual().unstarted("server-socket") {
            public void run() {
                while (!isInterrupted()) {
                    try {
        ...
        for (int i = 0; i < count; i++) {
            ...
            Thread t = Thread.ofVirtual().unstarted("client-socket-" + i) {
                public void run() {
                    try {
    ...
    private void testDanglingSocketsOnTerminate(boolean withSocketInterceptor) throws Exception {
        ...
        for (int i = 0; i < count; i++) {
            ...
            Thread t = Thread.ofVirtual().unstarted(task, "socket-thread-" + i);
            t.start();
        }
